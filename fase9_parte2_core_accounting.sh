#!/bin/bash
# ============================================================
# Fase 9 (Asientos contables) - Parte 2: core/accounting.py
# ============================================================
# CAMBIOS: nuevo archivo apps/backend/app/core/accounting.py
#
# Cada generate_*_entry() devuelve un dict {error, lines, ...} SIN
# persistir nada - permite validar/mostrar antes de guardar. Si falta
# una cuenta contable requerida, error="missing_accounts" (patron
# "blocking cascade" de siempre, no se asume ni se inventa cuenta).
#
# Resolucion de cuentas (documentado en el docstring del archivo):
#   - Gasto de planilla ordinaria: Branch.accounting_account (ya
#     existia desde mod.6, pensado para esto).
#   - Pasivo de cada concepto (CCSS-EMPLEADO, RENTA, AGUINALDO,
#     CCSS-PATRONAL): PayrollConcept.accounting_account_id (nuevo).
...
#     "HORAS_EXTRA" ya usan en core/payroll.py - no es un valor
#     financiero, es una clave de busqueda).
#
# Cesantia: SOLO se genera al aprobar una Termination con
# responsabilidad patronal (decision confirmada) - no hay provision
# mensual especulativa.
#
# Ejecutar: cd /opt/workforce-ai-os && bash fase9_parte2_core_accounting.sh
# ============================================================
set -e
REPO_DIR="${REPO_DIR:-/opt/workforce-ai-os}"
cd "$REPO_DIR"

cat > apps/backend/app/core/accounting.py << 'PYEOF'
"""
...
echo "OK: apps/backend/app/core/accounting.py escrito"

python3 -m py_compile apps/backend/app/core/accounting.py
echo "SYNTAX OK: accounting.py"
