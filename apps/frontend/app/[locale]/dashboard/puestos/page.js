...
export default function PuestosPage() {
  ...
  async function handleCreate(e) {
    ...
    setCreateError(null);
    try {
      await apiFetch("/api/positions", { method: "POST", body: JSON.stringify({ name }) });
      setName("");
  ...
  async function handleSaveEdit(e) {
    ...
    setEditError(null);
    try {
      await apiFetch("/api/positions/" + selected.id, {
        method: "PATCH",
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-bk-brown/10 overflow-hidden">
            {loading ? (
              ...
              <EmptyState icon={Briefcase} message={t("no_data")} />
            ) : (
              <ul className="divide-y divide-bk-brown/10">
                {filtered.map((p) => (
                  <li key={p.id}>
                    ...
                    >
                      <div>
                        <p className="font-semibold text-bk-brown text-sm">{p.name}</p>
                        <p className="text-xs text-bk-brown/60">{t("employee_count", { count: p.employee_count })}</p>
                      </div>
                      <span
                        className={
                          p.active
          ...
          {canManage && (
            ...
            >
              <h3 className="font-heading font-bold text-bk-brown">{t("new_position")}</h3>
              {createError && (
        ...
        <div>
          {selected ? (
            <form
              ...
              className="bg-white rounded-xl shadow-sm border border-bk-brown/10 p-5 space-y-3 text-sm"
            >
              <h3 className="font-heading font-bold text-bk-brown">{t("edit_position")}</h3>
              {editError && (
