...
- 🟢 **Mód. 15 (fases 1-6 de 11, post-MVP)** — Reporte de horas trabajadas, Nómina bruta (`PayrollHoursConfig`), campo `pay_frequency` en `Contract`, Calendario de planillas (`PayrollPeriod`), Horas extra con aprobación (`OvertimeApproval`), Catálogo de feriados (`Holiday`) y Deducciones CCSS + Renta → Neto (`core/renta.py`: CCSS vía `PayrollConcept`, renta con tramos progresivos + créditos por dependiente, mensual y quincenal con mecánica de acumulado). Detalle completo del porqué y las correcciones de proceso en sección 0.
