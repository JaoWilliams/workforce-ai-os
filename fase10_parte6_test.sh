#!/bin/bash
# ============================================================
# Fase 10 (Archivo bancario) - Parte 6: test end-to-end
# ============================================================
# Crea el test en el host, lo copia dentro del contenedor y lo corre.
# Cubre: config faltante (bloqueo), config real (glosa confirmada),
# cada rama del patron blocking-cascade (neto no computable, empleado
# no encontrado, neto cero/negativo, cuenta bancaria faltante, caso
# valido), persistencia, formato exacto del TXT, los 4 endpoints
# reales, y el caso no_valid_rows.
# Ejecutar: cd /opt/workforce-ai-os && bash fase10_parte6_test.sh
# ============================================================
set -e
REPO_DIR="${REPO_DIR:-/opt/workforce-ai-os}"
cd "$REPO_DIR"

cat > test_bank_file_e2e.py << 'PYEOF'
"""
...
echo "=== copiando el test dentro del contenedor ==="
docker compose cp test_bank_file_e2e.py api:/app/test_bank_file_e2e.py

echo "=== ejecutando ==="
docker compose exec -T api python3 test_bank_file_e2e.py

echo "=== FIN Parte 6 (test) ==="
